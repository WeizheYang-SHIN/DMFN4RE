...
def load_model_for_prediction(model_path):
    ...
    hparams.mode = 'predict'  # 强制设置为预测模式

    # 初始化模型
    model = ASLREModel(hparams, checkpoint['vocab'])
    model.load_state_dict(checkpoint['model_state_dict'])

    return model

def predict(hparams, test_data, vocab):
    ...
    # model = ASLREModel(hparams,vocab).to(device)
    ...
    with torch.no_grad():
        ...
        for test_i_batch, test_sample_batched in enumerate(tqdm(test_loader,desc='testing')):
            ...
            tag_ids = test_sample_batched['tag_id'].to(device)
            # logits = model(token_ids, token_type_ids, attention_mask, e1_mask, e2_mask).to(device)  # 验证集上的预测结果
            logits = model(token_ids, e1_mask, e2_mask).to(device)  # 验证集上的预测结果
            logits = logits.to(device)
            pred_tag_ids = logits.argmax(1).to(device)
